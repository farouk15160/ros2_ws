...
RgbdSlamNode::RgbdSlamNode(ORB_SLAM2::System *pSLAM)
  ...
  rgb_sub = std::make_shared<message_filters::Subscriber<ImageMsg>>(
      shared_ptr<rclcpp::Node>(this), "/camera/color/image_raw");
  depth_sub = std::make_shared<message_filters::Subscriber<ImageMsg>>(
      shared_ptr<rclcpp::Node>(this), "/camera/depth/image_raw");
  ...
  pub_rgb =
      this->create_publisher<sensor_msgs::msg::Image>("/RGBD/RGB/Image", 10);
  pub_depth =
      this->create_publisher<sensor_msgs::msg::Image>("/RGBD/Depth/Image", 10);
  pub_tcw = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      "/RGBD/CameraPose", 10);
  pub_odom =
      this->create_publisher<nav_msgs::msg::Odometry>("/RGBD/Odometry", 10);
  pub_camerapath =
      this->create_publisher<nav_msgs::msg::Path>("/RGBD/Path", 10);
}
...
RgbdSlamNode::~RgbdSlamNode() {
  ...
  m_SLAM->SaveKeyFrameTrajectoryTUM(
      "/root/yahboomcar_ros2_ws/software/library_ws/src/ros2-ORB_SLAM2/src/"
      "rgbd_pose/KeyFrameTrajectory.txt");
}
...
void RgbdSlamNode::GrabRGBD(const ImageMsg::SharedPtr msgRGB,
  ...
  // cv::Mat Tcw = m_SLAM->TrackRGBD(cv_ptrRGB->image, cv_ptrD->image,
  // msgRGB->header.stamp.sec);
  cv::Mat Tcw = m_SLAM->TrackRGBD(cv_ptrRGB->image, cv_ptrD->image,
                                  msgRGB->header.stamp.sec);
  isKeyFrame = m_SLAM->MapChanged();
  if (!Tcw.empty()) {
    cv::Mat Twc = Tcw.inv();
    ...
    // RCLCPP_INFO(this->get_logger(), "tWC.at<float>(0): %f",
    // tWC.at<float>(0));
    if (isKeyFrame) {
